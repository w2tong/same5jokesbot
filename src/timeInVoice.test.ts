...
describe('userJoin()', () => {
    test('single user', () => {
        ...
        expect(timeInVoice.userJoinTime['user1']).toEqual({channelId: 'channel1', guildId: 'guild1', time: mockDate1.getTime()});
    });
    test('same user', () => {
        timeInVoice.userJoin('user2', 'channel1', 'guild1');
        expect(timeInVoice.userJoinTime['user2']).toEqual({channelId: 'channel1', guildId: 'guild1', time: mockDate1.getTime()});
        timeInVoice.userJoin('user2', 'channel2', 'guild2');
        expect(timeInVoice.userJoinTime['user2']).toEqual({channelId: 'channel2', guildId: 'guild2', time: mockDate1.getTime()});
    });
    test('multiple users', () => {
        ...
        expect(timeInVoice.userJoinTime['user3']).toEqual({channelId: 'channel2', guildId: 'guild3', time: mockDate1.getTime()});
        expect(timeInVoice.userJoinTime['user4']).toEqual({channelId: 'channel1', guildId: 'guild1', time: mockDate1.getTime()});
        expect(timeInVoice.userJoinTime['user5']).toEqual({channelId: 'channel2', guildId: 'guild1', time: mockDate1.getTime()});
        expect(timeInVoice.userJoinTime['user6']).toEqual({channelId: 'channel1', guildId: 'guild2', time: mockDate1.getTime()});
    });
    test('no user', () => {
...
describe('userChangeChannel()', () => {
    ...
    test('single user single change', async () => {        
        ...
        expect(timeInVoice.userJoinTime['user1']).toEqual({channelId: 'channel2', guildId: 'guild1', time: mockDate2.getTime()});
    });
    test('single user multiple changes', async () => {        
        ...
        expect(timeInVoice.userJoinTime['user1']).toEqual({channelId: 'channel3', guildId: 'guild1', time: mockDate2.getTime()});
    });
    test('multiple users', async () => {        
        ...
        expect(timeInVoice.userJoinTime['user1']).toEqual({channelId: 'channel1', guildId: 'guild1', time: mockDate2.getTime()});
        expect(timeInVoice.userJoinTime['user2']).toEqual({channelId: 'channel2', guildId: 'guild2', time: mockDate2.getTime()});
        expect(timeInVoice.userJoinTime['user3']).toEqual({channelId: 'channel3', guildId: 'guild3', time: mockDate2.getTime()});
    });
});
