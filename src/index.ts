...
client.on('messageCreate', async (message: Message): Promise<void> => {
	...
	if (message.member && message.member.roles.cache.some((role) => role.name === 'Bot Abuser'))
		return;
...
client.on('interactionCreate', async (interaction: Interaction): Promise<void> => {
	...
	if (
		...
		const reply = interaction.member.voice
			? `Playing ${interaction.options.getString('audio')}.`
